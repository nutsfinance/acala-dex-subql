...
export const addHourProvisionPool = async (hash: string, hourTime: Date, poolId: string, token0: string, token1: string, token0Amount: bigint, token1Amount: bigint) => {
	...
	hourProvisionPool.price0 = BigInt(parseInt(hourProvisionPool.token0Amount.toString()) / parseInt(hourProvisionPool.token1Amount.toString()));
	hourProvisionPool.price1 = BigInt(parseInt(hourProvisionPool.token1Amount.toString()) / parseInt(hourProvisionPool.token0Amount.toString()));
...
export const createAddProvisionHistory = async (event: SubstrateEvent, addressId: string, poolId: string, token0: string, token1: string, token0Amount: bigint, token1Amount: bigint) => {
	...
	history.price0 = BigInt(FN.fromInner(parseInt(token0Amount.toString()) / parseInt(token1Amount.toString()), 18).toChainData());
	history.price1 = BigInt(FN.fromInner(parseInt(token1Amount.toString()) / parseInt(token0Amount.toString()), 18).toChainData());
