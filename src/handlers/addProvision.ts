...
export const createAddProvisionHistory = async (event: SubstrateEvent, addressId: string, poolId: string, token0: string, token1: string, token0Amount: bigint, token1Amount: bigint) => {
	...
	history.price0 = BigInt(parseInt(token0Amount.toString()) / parseInt(token1Amount.toString()));
	history.price1 = BigInt(parseInt(token1Amount.toString()) / parseInt(token0Amount.toString()));
