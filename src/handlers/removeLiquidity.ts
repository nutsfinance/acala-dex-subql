...
export const createRemoveLiquidyHistory = async (event: SubstrateEvent, price0: FN, price1: FN) => {
	...
	history.price0 = BigInt(FN.fromInner(parseInt(token0Decrement.toString()) / parseInt(token1Decrement.toString()), 18).toChainData());
	history.price1 = BigInt(FN.fromInner(parseInt(token1Decrement.toString()) / parseInt(token0Decrement.toString()), 18).toChainData());
